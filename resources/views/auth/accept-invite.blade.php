...
<title>Activate Account — DP-LMS</title>
...
.user-badge svg { width:15px; height:15px; flex-shrink:0; }
...
/* ── pw-input — class-based so type swap doesn't break styles ── */
.pw-input {
...
.pw-input.is-error { border-color:#fca5a5 !important; background:#fff5f5 !important; }
.pw-input.is-valid { border-color:#86efac !important; background:#f0fdf4 !important; }
...
.pw-toggle:active { background:rgba(230,57,70,.12); }
...
.eye-on  { opacity:1; transform:scale(1);    pointer-events:none; }
.eye-off { opacity:0; transform:scale(0.75); pointer-events:none; }

.pw-toggle.is-visible .eye-on  { opacity:0; transform:scale(0.75); }
.pw-toggle.is-visible .eye-off { opacity:1; transform:scale(1);    }
...
.pw-strength-lbl { font-size:.7rem; height:14px; transition:color .25s; }
...
<div class="wrapper">
  ...
  <div class="card" id="inviteCard">
    ...
    <p class="card-sub">
      Welcome, <strong>{{ $teacher->name }}</strong>! Create a secure password to activate your
      DP-LMS teacher account.
    </p>
    ...
    <form method="POST" action="{{ route('invite.accept', $token) }}" id="inviteForm" novalidate>
      @csrf

      {{-- New Password --}}
      <div class="form-group" id="fg1">
        <label for="pwInput">New Password <span style="color:var(--red)">*</span></label>
        <div class="input-wrapper">
          ...
          <button type="button" class="pw-toggle" id="togglePw1"
            ...
            {{-- Eye open --}}
            <svg class="eye-on" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" aria-hidden="true">
              <path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.964-7.178z"/>
              <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
            </svg>
            {{-- Eye off --}}
            <svg class="eye-off" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" aria-hidden="true">
      ...
      <div class="form-group" id="fg2">
        <label for="pwConfirm">Confirm Password <span style="color:var(--red)">*</span></label>
        <div class="input-wrapper">
          <span class="input-icon">
            <svg fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
              <path stroke-linecap="round" stroke-linejoin="round"
      ...
      <button type="submit" class="btn-submit" id="submitBtn">
        ...
        <div class="spinner" id="btnSpinner" role="status" aria-label="Activating"></div>
...
<script>
// ── Toggle — class-based state, CSS handles crossfade
...
function updateStrength(val) {
  ...
  [1,2,3,4].forEach(i => {
    document.getElementById('seg'+i).style.background = i <= s ? colors[s-1] : '#e5e7eb';
  });
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (reduced) {
    ...
    gsap.to(['#inviteCard','#copyright'], { opacity:1, duration:.3, stagger:.06 });
    gsap.to('.form-group', { opacity:1, y:0, duration:.3, stagger:.07 });
  } else {
    const tl = gsap.timeline({ defaults:{ ease:'power3.out' } });
    tl.to('body',       { opacity:1, duration:.35 })
      .to('#inviteCard', { opacity:1, y:0, scale:1, duration:.5 }, '-=.2')
